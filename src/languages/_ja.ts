...
const ja = {
  index: {
      ...
      secure_body2: 'Symbolは4層からなるネットワークで構成されています。バックボーンを維持しチェーンの分散化や状態記録を行いブロックを生成する「PeerNode」、各ウォレットからの取引要求や履歴を処理し、データをPeerNodeと通信する「APINode」、APINode上で各ウォレットからの処理要求を受付する「Rest Gateway」、各ウォレットやSDKである「Client」からなるネットワークをモジュール化して展開する事で、セキュリティの担保や容易なアップデートを可能にしています。',
